...
class TestCraftingCalculatorPc178:
    @pytest.mark.parametrize(
        ...
        load_test_cases(os.path.join(os.path.dirname(__file__), "test_cases", "pc178.json")),
    )
    def test_calculation(self, seed_str, pickups, correct_id, items_unlocked):
        ...
        if items_unlocked:
            assert correct_id in item_ids, f"Expected {correct_id} but got {item_ids[0]} for {pickups}"
        else:
            assert correct_id in item_ids, f"Expected {correct_id} but got {item_ids[1]} for {pickups}"
