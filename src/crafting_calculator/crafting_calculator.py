...
def main():
    ...
    parser.add_argument('--pickups', required=True, metavar='ID', type=int, nargs="+",
                        help='Use these pickup IDs in calculation. If calculating a single item to create, you must pass 8 IDs. The following are all the valid IDs: ' + ', '.join(
                            [f'{pickup.pickup_id} - {pickup.pickup_name}' for pickup in PICKUP_LIST if
                             pickup is not None]))
    group = parser.add_mutually_exclusive_group()
    group.add_argument('--find-pickup-recipes', action="store_true",
                       help="Find all recipes that use these pickup types.")
    group.add_argument('--find-item-recipes', metavar='ITEM_ID', type=int,
                       help="Find all recipes matching this item ID for the given pickup types (sorted by quality).")
    group.add_argument('--find-uncraftable-items', action="store_true",
